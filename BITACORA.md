...
## Adenda 4: la primera sección de Historia COMPLETA, y dos erratas del examen original

El `2013-1op-2-2013` (página 74, sello del 20 JUL 2013) pasó de 14 a **24 preguntas**: las diez de Historia se pudieron transcribir, porque las diez son hechos verificables contra el registro histórico. El facsímil está limpio, sin marcas a mano.

Con esto hay dos exámenes de Económicas con las tres áreas, y este es el primero con la sección de Historia entera.

## Dos erratas del examen original, que se transcriben y se aclaran

- **El decreto de despojo de tierras comunarias fechado en "1800".** Es imposible: en 1800 Bolivia no existía como república. El decreto que el enunciado describe, con su plazo de dos meses y sus títulos de entre 25 y 100 pesos, es el de Mariano Melgarejo del 20 de marzo de **1866**. El resto del enunciado lo identifica sin ambigüedad, así que la pregunta es resoluble y se transcribió con la aclaración en la explicación.
- **"José Luis Tejada Serrano".** El apellido es **Sorzano**, y el mismo personaje aparece bien escrito en el `2013-2op-2`. Se transcribe como está impreso y se aclara.

Y una tercera cosa que no es errata pero conviene saber: la pregunta del Romanticismo ofrece **"Existencialismo" dos veces**, en la c) y en la e).

## El test de voseo y su tercera colisión: un NOMBRE PROPIO

El test frenó la palabra *"tomás"* en **"Tomás Alva Edison"**. No es voseo: es el nombre del inventor, que coincide letra por letra con la forma vosea de *tomar*.

Ya había dos colisiones anotadas y las dos eran sustantivos comunes (*tomate* de "tomar"+"te", *terminales* de "terminar"+"les"). Esta es distinta porque **se distingue por la mayúscula**: un verbo conjugado en medio de una frase no la lleva. Así que la excepción se agregó como una lista de nombres propios chequeada **de forma sensible a mayúsculas**, a diferencia del regex, que es case-insensitive a propósito.

Y se probó que la excepción no tapa un voseo real: se inyectó *"Si tomás nota de..."* en minúscula y el test volvió a ponerse rojo. Después se revirtió.

## El revert que se llevó puesto el trabajo

Al revertir esa inyección se usó `git checkout -- <archivo>`, y eso **descarta TODO lo no commiteado de ese archivo**, no solo el último cambio: se borraron de un golpe las diez preguntas de Historia recién escritas.

Se repusieron corriendo de nuevo el script que las había generado, y eso es lo que salvó la situación. **Escribir cada tanda como un script con `assert` en vez de editar a mano tiene este beneficio que no era el buscado: el trabajo se puede reproducir.** Si hubiera sido una edición manual, había que rehacer las diez.

Para la próxima: para deshacer una inyección de prueba, revertir la cadena exacta que se inyectó, no el archivo.
...
Se cerraron las 88. Verificado con el script: de la lista regenerada, las 88 están auditadas y ninguna auditada quedó fuera de ella (`faltan 0`, `sobran 0`).

## El resultado, en una línea

**Las 88 respuestas están bien.** Los 15 problemas encontrados son **todos de texto**.

| modo | qué pasó | cuántos |
|---|---|---|
| **1 · la respuesta está mal** | — | **0** |
...
Las dos respuestas mal que se venían citando (`2016-1op-1` P7 y `2016-3op-1` P6) son de un muestreo **anterior** y no están en esta lista.

## Lo que eso dice del banco

**Quien transcribió resolvió bien.** Tenía la figura delante y la usó: en varios casos la *explicación* del archivo usa un dato que la *descripción* no menciona (el más claro, la colinealidad A-C-D de las dos `2018-1op` P5). Lo que se rompió no fue el trabajo original: fue la traducción de la figura a palabras, y la rompieron **pasadas automáticas posteriores**.

Es la misma conclusión a la que llegaron, por otro camino, los 14 enunciados vacíos del 17-sep. Tres veces seguidas el daño vino del mismo lugar. Si de todo esto queda una sola cosa escrita, que sea: **editar el banco en masa sin verificar el resultado en masa es la forma más eficiente que encontramos de romperlo.**

## Las dos que justifican el trabajo

- **`2015-1op-2` P8** preguntaba *"la distancia AB es igual a"* **sin que A ni B aparecieran en el texto**. Solo existían en el dibujo. Un alumno no podía ni empezar.
- **`2025-final-1` P8** es el único caso donde el dato faltante **cambiaba la respuesta**: sin saber que el globo está entre A y B, la lectura natural da 64,6 km, que no está entre las opciones y llevaría a marcar "Ninguno".

## Diez figuras, tres compartidas

Se dibujaron 10 figuras. Tres las comparten dos preguntas cada una: los bloques apilados de 2013 (el 2do parcial trae kilos y el final trae razones, y el dibujo no lleva números), el rombo de 2015 (una pregunta pide el área y la otra el perímetro) y el tazón que el `2015-1op-2` P12 reusa del final 2-2014. **En los tres se compararon las imágenes de los facsímiles antes de compartir el id**, siguiendo la lección del lote 2017.

La que NO se compartió es interesante por lo contrario: las dos `2018-1op` P5 son **exactamente el mismo dibujo**, pero una acota AB, AF, CD y DE y la otra AB, AC, DF y DE. Se hicieron dos constructores, porque **una figura que muestre cotas que el enunciado no da enseña mal**.

## La regla que salió del final

El **`2024-parcial2-2` P10** parecía indeterminado —falta la altura de la bandera— y no lo estaba: se resuelve probando las opciones, porque el par (θ, d) tiene que cumplir tan θ = 16/(d+9) y solo una de las cuatro lo cumple. **Antes de declarar que una pregunta no se puede resolver, probar si las opciones la cierran.**

### 2026-09-18 (quater) (corrección del conteo de la auditoría: son 50 de 88, no 58)

Las entradas anteriores de hoy dicen que van 44 y después 58 auditadas de las 88. **Son 50**, y faltan 38.

## De dónde salía el error

Se estaban sumando 8 preguntas de un **muestreo anterior** (`2016-1op-1` P7, `2016-3op-1` P6 y seis de 2017) que **no forman parte de las 88**: esas ya tenían su figura declarada, así que el filtro que define la lista no las incluye. Contarlas como progreso de las 88 inflaba el avance.

Se recalculó con el script, cruzando el conjunto de auditadas contra la lista regenerada: 50 de 88 están en la lista, ninguna auditada quedó fuera de ella, y faltan 38.

## Y el error tapaba una buena noticia

Las **2 respuestas equivocadas** que se venían arrastrando en el conteo (*"solo 2 son respuestas mal"*) son justamente las dos del muestreo anterior. De las **50 de la lista auditadas hasta ahora, las 50 tienen la respuesta bien.** Los 14 problemas encontrados son **todos de texto**: enunciados rotos, descripciones que no son la figura, y datos que la figura tenía y el texto perdió.

Eso refuerza lo que ya se venía viendo y conviene decirlo sin el número inflado: **el banco responde bien; lo dañado es la capa de palabras que se le puso encima.**

## La causa, que es la misma de la mañana

Es el segundo error de conteo del día (el primero fue *"tres de cuatro respuestas mal"* cuando eran dos). Los dos salieron de lo mismo: **escribir un número de memoria en vez de calcularlo.** Para los conteos de esta auditoría hay un script que cruza las listas; usarlo cuesta una corrida y evita tener que corregir el changelog dos veces.
...
### 2026-09-17 (quinquies) (las cuatro de física, y dos respuestas que estaban mal)

**5 → 1.** Se terminó la lista: las cuatro que quedaban eran de física, una por facsímil. **Dos de las cuatro estaban mal respondidas**, y son justo los dos circuitos: en los dos, el propio archivo admitía que había adivinado la figura. Las otras dos (las del 2014) ya estaban bien.

> **Corrección (18-sep):** la primera versión de esta entrada y de `docs/figuras-pendientes.md` decían *"tres de cuatro estaban mal respondidas"*. Son **dos**: se verificó con `git show 7a96617 -- data/examenes | grep respuesta` y solo hay dos líneas de respuesta cambiadas (B→C y E→A). Queda anotado porque el número exagerado inflaba la tasa de error del banco, que es justo el dato con el que se decide cuánto revisar.
