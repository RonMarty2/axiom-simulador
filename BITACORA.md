...
- [ ] **29 enunciados nombran una figura que no existe, y necesitan el facsímil.** Arrancó en 130 el 14-sep: 106 se resolvieron sin abrir un PDF (98 reescribiendo el enunciado, que ya traía la configuración, y 14 dibujando la figura cuando los datos la determinaban), y el 16-sep se dibujaron 12 más leyendo los facsímiles en local — los cinco PDF de la gestión 1-2016, que quedó terminada. Las 29 que quedan no se pueden reconstruir del texto. Qué hace falta para cada una, y las tres formas de desbloquearlo, están en [`docs/figuras-pendientes.md`](../docs/figuras-pendientes.md). El test `ningún enunciado nuevo promete una figura que no está` tiene el tope en 29 y solo puede bajar.
- [ ] **73 preguntas más que el trinquete no ve, y hay que contrastar contra su facsímil.** Son las que la pasada del 14-sep sacó de la cuenta reescribiendo el enunciado, y que hoy no declaran `figura:`. La mayoría probablemente estén bien (el texto trae la configuración en un paréntesis), pero nadie las verificó contra el PDF y **las dos primeras que se abrieron estaban mal**: `2016-1op-1` P7 tenía la geometría descrita al revés y `2016-3op-1` P6 necesitaba el dibujo para saber desde qué borde se mide la sombra. Lista completa y definición exacta del filtro en [`docs/figuras-pendientes.md`](../docs/figuras-pendientes.md), sección "El 29 es un piso, no un techo".
- [ ] **Las verificaciones geométricas de las figuras no corren en CI.** `verificarAngulo`/`verificarDistancia` explotan al CONSTRUIR la figura, y nada la construye en los tests: `banco.test.ts` lee el registro de `definiciones.ts` como texto, porque ese módulo importa `./motor` sin extensión y `node --test` corre ESM, donde la extensión es obligatoria. Una figura con la verificación rota se commitea sin que nada avise y explota recién en el navegador, en esa sola pregunta. Se arregla sin tocar los imports de la app: copiar el módulo a un temp con la extensión puesta, importarlo y construir los ~49 ids del registro (es lo que hizo a mano el harness del 16-sep).
