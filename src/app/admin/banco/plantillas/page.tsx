...
IMPORTANTE: ese archivo lo va a leer un PROGRAMA, no una persona. Si cambias el nombre de un campo, un símbolo de la estructura o el orden de las partes, el programa lo rechaza. No inventes, no resumas, no "mejores" nada del original.
...
1. FIDELIDAD TOTAL. Transcribí cada enunciado y cada opción EXACTAMENTE como aparece en el PDF. Sin sinónimos, sin redondeos, sin reordenar opciones. Si el PDF dice sen 37° = 3/5, escribe 3/5 (no 0,6). Si una opción dice "NINGUNO", escribe "Ninguno".

2. NO ADIVINES NUNCA. Si un número, palabra o parte de una figura no se lee con claridad, escribe "VERIFICAR: [qué cosa no se lee]" dentro de la explicación de esa pregunta y sigue con la siguiente. Preferimos un hueco marcado a un dato inventado.

3. EL PROBLEMA ES EL QUE MUESTRA LA FIGURA, NO EL QUE TE SUENA PARECIDO. Antes de resolver una pregunta con dibujo, mira el dibujo y declarà qué es (¿una masa colgando de un hilo? ¿un bloque sobre un plano? ¿un circuito?). Error real que ya pasó: una IA resolvió "bloque sobre plano inclinado" cuando la figura mostraba un PÉNDULO colgando de un hilo; el número final coincidía de casualidad y el error casi queda. Enunciado, figura y solución tienen que contar EL MISMO problema.

4. VERIFICÁ CADA RESPUESTA ANTES DE MARCARLA. Resuelve la pregunta paso a paso y confirma que TU resultado coincide con la opción que marcas. Si no coincide con NINGUNA opción, la respuesta es E (Ninguno) — NO fuerces tu resultado hacia la opción "más parecida". En estos exámenes "Ninguno" es respuesta correcta REAL varias veces (en un solo examen nos pasó 3 veces): las demás opciones están elegidas a propósito "con pinta de correctas" para cazar al que no confía en su cálculo.

5. LAS EXPLICACIONES ENSEÑAN, NO SOLO MUESTRAN LA CUENTA. Cada vez que un paso use un teorema o propiedad (ángulos alternos internos, correspondientes, ángulo exterior, suma de ángulos de un triángulo, Pitágoras, regla de tres, presión osmótica, balanceo redox, lo que sea), recuerda en UNA frase qué dice esa propiedad ANTES de usarla — el alumno no tiene por qué tenerla fresca. Ejemplo: en vez de "por alternos internos vale 40°", escribe "recuerda: cuando una recta corta a dos paralelas, los ángulos que quedan entre las paralelas a lados opuestos son iguales (forman una Z); por eso este ángulo también vale 40°". Si existe un atajo que ahorra tiempo frente al método largo, decilo explícitamente ("atajo: ...").
...
Sobre "ponderacion": una línea por cada ÁREA/SECCIÓN que el PDF realmente tenga, con dos espacios de sangría. Mira los títulos de sección del PDF: en la UMSS, Aritmética-Álgebra y Geometría-Trigonometría suelen ser DOS secciones distintas (no una sola "matemáticas"). Los nombres de área van en snake_case: aritmetica_algebra, geometria_trigonometria, fisica, quimica, biologia. Los pesos deben sumar 1.0 (si el PDF no indica pesos, reparte igual entre las áreas).
...
[SOLO si la pregunta tiene dibujo, agrega acá este bloque:
...
[Y SIEMPRE que la pregunta tenga dibujo, inmediatamente después del bloque FIGURA agrega el DIBUJO EN CÓDIGO SVG: la etiqueta <svg ...> ... </svg> pegada directamente en el texto (sin envolverla en nada). Este SVG es lo que verá el alumno, así que dibujalo IDÉNTICO al del PDF. Reglas del SVG:
- Empieza con <svg viewBox="0 0 420 240" xmlns="http://www.w3.org/2000/svg"> y terminá con </svg>. Si la figura es más alta que ancha, usa viewBox="0 0 420 300".
...
- IDENTIFICÁ BIEN CUÁL ÁNGULO ES EL MARCADO. Donde se cruzan dos rectas se forman CUATRO ángulos (dos pares opuestos). Antes de codificar el arco, mira con cuidado ENTRE QUÉ DOS RAYOS exactos va el arco en la imagen (¿arriba o abajo del cruce? ¿a la izquierda o a la derecha?) y anotalo así de preciso en tu propia descripción FIGURA antes de dibujar. Un arco puesto en el par de ángulos equivocado mide los grados correctos pero queda dibujado en el lugar incorrecto — error real que ya pasó.
...
- TODO DENTRO DEL VIEWBOX. Antes de terminar, repasa las coordenadas de cada punto que usaste: ninguna puede ser negativa ni superar el ancho/alto que declaraste, y deja un margen de al menos 8-10 unidades en los bordes. Un elemento que se sale del viewBox se recorta o desaparece en el navegador del alumno.
...
- NO ASUMAS UNA DIRECCIÓN POR "CÓMO SUELE SER" EL PROBLEMA. Si el problema es de física y tu intuición dice que un vector/campo "debería" apuntar de tal manera, IGUAL mira la imagen y dibuja la dirección que REALMENTE se ve ahí, aunque te parezca rara. La imagen manda siempre sobre cualquier expectativa previa.
- Repasa tu SVG contra tu propia descripción FIGURA punto por punto ((a) a (f)) antes de continuar.]
...
========== ENTREGA ==========
- Si puedes, entregá el archivo COMPLETO en una sola respuesta.
- Si el examen es demasiado largo para tu límite de respuesta: entregá primero el encabezado + las preguntas 1 a 5, y cuando el usuario escriba "sigue", continuá con las 5 siguientes SIN repetir el encabezado y manteniendo la numeración. Nunca cortes una pregunta por la mitad.
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ background: "linear-gradient(135deg, rgba(99,102,241,0.10), rgba(168,85,247,0.10))", borderRadius: 14, padding: 20, border: "2px solid var(--accent)", marginBottom: 18 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 17, fontWeight: 800, color: "var(--fg-primary)" }}>Megaprompt AXIOM (el único que necesitas)</div>
              <div style={{ fontSize: 13, color: "var(--fg-muted)" }}>
                Cubre los dos usos: <strong>examen oficial completo</strong> (le adjuntas el PDF y te devuelve todo transcripto y resuelto paso a paso) y <strong>preguntas de práctica</strong> (le pides &ldquo;20 preguntas sobre ángulos entre paralelas&rdquo;). Siempre en el formato exacto de AXIOM: soluciones que enseñan, fórmulas en LaTeX y figuras descritas al detalle.
              </div>
            </div>
        ...
        <div style={{ padding: 20, background: "rgba(99,102,241,0.06)", borderRadius: 14, border: "1px solid var(--border)", marginBottom: 18 }}>
          ...
          <ol style={{ paddingLeft: 20, lineHeight: 1.7, color: "var(--fg-primary)", fontSize: 14 }}>
            ...
            <li><strong>Modo examen:</strong> adjuntá el PDF (o fotos) del examen pasado. <strong>Modo práctica:</strong> escribe abajo del prompt &ldquo;dame 20 preguntas sobre [tema]&rdquo;.</li>
            <li>Pega la respuesta de la IA en <Link href="/admin/banco/subir-examen" style={{ color: "var(--accent)", fontWeight: 700 }}>Subir examen</Link>: se valida sola, se guarda en el banco, y con doble clic en <strong>SUBIR</strong> viaja a GitHub para que Claude la revise, dibuje las figuras con el motor de geometría y la integre. (Las preguntas de práctica sueltas también se pueden pegar en <Link href="/admin/banco/importar" style={{ color: "var(--accent)", fontWeight: 700 }}>Importar</Link>.)</li>
          </ol>
        </div>
        ...
        <details style={{ background: "var(--bg-card)", borderRadius: 14, padding: 18, border: "1px solid var(--border)" }}>
          <summary style={{ cursor: "pointer", fontWeight: 700, color: "var(--fg-primary)", fontSize: 14 }}>
            📦 ¿Ya tienes preguntas hechas en otro formato? (Excel, Moodle, JSON…)
          </summary>
          <p style={{ fontSize: 13, color: "var(--fg-muted)", margin: "10px 0 12px" }}>
            Descargá el ejemplo del formato que uses para ver cómo estructurarlo, y después pega tus preguntas en <Link href="/admin/banco/importar" style={{ color: "var(--accent)", fontWeight: 700 }}>Importar</Link>.
          </p>
